...
async function getContent() {
  try {
    // 1. Intentamos traer los datos reales desde la base de datos en Hostinger
    const response = await fetch('api.php');
    if (response.ok) {
      const data = await response.json();
      // Verificamos si la base de datos ya tiene la clave del contenido
    ...
    console.error("Error al conectar con Hostinger, usando plan B:", error);
  }

  // 2. Si el servidor falla o está vacío, usamos el localStorage como respaldo
  const local = loadLocalContent();
  if (local) return local;

  // 3. Si todo lo demás falla, usamos los datos por defecto del código
